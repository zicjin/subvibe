---
description: Codex-orchestrated deep research — Antigravity (agy/Gemini) does the grounded web legwork; Codex plans, verifies the citations, and synthesizes.
argument-hint: "<what to research>"
---

Run a multi-source research pass on the topic below, following the **Deep-research
recipe** and **Verification gates** in the antigravity section of AGENTS.md.
Antigravity (`agy` / Gemini) is the cheap, grounded search worker; **you (Codex) own
the plan, the verification, and the synthesis**. agy's print-mode citations are coarse
(often domain-level) and it can present parametric "knowledge" as a sourced fact — so
never ship its citations unchecked.
...
Keep your own context lean — ingest agy's bullet digests, not the raw pages (that's where the cost savings come from). `--print` does one agentic pass per call, so re-dispatch follow-up agy calls to close gaps rather than expecting it to auto-iterate. In an interactive session a long fetch can be backgrounded with `agy-job`; when **you** are headless (`codex exec`), delegate synchronously.
